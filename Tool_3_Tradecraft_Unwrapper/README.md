...
## Version 1.1.0 Feedback Updates

Version 1.1.0 adds two peer-feedback-driven usability improvements.

First, `--scan-embedded` performs an optional scan for Base64 and hexadecimal fragments embedded inside longer text. This addresses reviewer feedback that whole-stage decoding alone can miss encoded material when the encoded value appears inside a larger command or script line. Embedded candidates are reported with type, offset, confidence, decoded size, evidence, and decoded preview so the analyst can review the candidate instead of treating it as an automatic conclusion.

Example:

```bash
python3 tradecraft_unwrapper.py \
  --text '$s = "SUVYIChOZXctT2JqZWN0IE5ldC5XZWJDbGllbnQpLkRvd25sb2FkU3RyaW5nKCJodHRwOi8vZXZpbC94Iik="' \
  --scan-embedded \
  -o output/embedded-demo
```

Second, `--simple` provides a faster analyst workflow for one-off text decoding. When used with `--text`, the tool prints the best decoded stage directly to the terminal while still allowing normal reports to be written when an output directory is supplied.

Example:

```bash
python3 tradecraft_unwrapper.py \
  --text 'SUVYIChOZXctT2JqZWN0IE5ldC5XZWJDbGllbnQpLkRvd25sb2FkU3RyaW5nKCJodHRwOi8vZXZpbC94Iik=' \
  --simple \
  -o output/simple-mode-validation
```

The automated test suite now includes 71 tests.
